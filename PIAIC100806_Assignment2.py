...
def function2():
    ...
    x = np.arange(1,37,dtype=np.float64).reshape((3,3,3))    
...
def function14():
    ...
    ans = np.array_split(arr,4,axis=0) 
...
def function16():
    ...
    x = np.array([[1], [2], [3]])
    y = np.array([[2], [3], [4]])
    ans = arr=np.concatenate((x,y),axis = 1) 
...
def function18():
    ...
    x = np.count_nonzero(students<100, axis = 0)
    return x
